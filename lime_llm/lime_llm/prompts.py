...
If you cannot flip sentiment while anchoring the mask=1 words, still attempt the strongest polarity shift possible.""",
        # Gemini
        "cola": """# DATASET CONTEXT: CoLA (Corpus of Linguistic Acceptability)
The input is a sentence; the Label indicates grammatical acceptability (0=Unacceptable, 1=Acceptable).

# STRATEGY DEFINITIONS
1. neutral_infill (Preserve Label):
   - If Original is ACCEPTABLE: Replace mask=0 words with synonyms or alternative phrasing that keeps the sentence grammatically correct.
   - If Original is UNACCEPTABLE: You MUST preserve the grammatical error. Do not fix the sentence. Replace mask=0 words in a way that maintains the specific syntactic violation.

2. boundary_infill (Flip Label):
   - If Original is ACCEPTABLE: Replace mask=0 words to introduce a specific syntactic error (e.g., subject-verb disagreement, missing argument, wrong case) to make it Unacceptable.
   - If Original is UNACCEPTABLE: Replace mask=0 words to CORRECT the error, making the sentence grammatically Acceptable.

# MASKING & INFILLING RULES
- 0-words are "changeable" slots. 1-words are fixed context.
- Intelligent Masking: For boundary_infill, you must mask the specific syntactic pivot points (e.g., the main verb, the auxiliary, or the subject) that allow you to flip the grammaticality.
- CRITICAL: Override your default behavior to "be helpful/correct." If the strategy requires an Unacceptable output, you must generate broken grammar.""",

        # ChatGPT52
        "hatexplain": """HateXplain is a 3-class toxic language classification task with word-level human rationales. Each input is a single short post/comment.

**Label meanings**

* **Normal**: non-toxic, non-harassing, no hateful or abusive intent.
* **Offensive**: rude, insulting, profane, or harassing language that is not clearly targeted hate toward a protected group.
* **Hate**: hateful or demeaning language directed at a protected group or identity, including dehumanization, calls for exclusion/violence, or group-based slurs.

**Mask semantics**

* **mask=1 words are fixed**: keep them exactly as-is and in the same order.
* **mask=0 words are changeable slots**: replace/infill them with new words/phrases (do not delete). Keep the sentence grammatical and natural.

**Strategy requirements**

* **neutral_infill**: preserve the given predicted label while changing only the mask=0 slots. Keep overall meaning, tone, and style close to the original; no topic drift; similar length.
* **boundary_infill**: aim for a different label by using the mask=0 slots to change the toxicity class. Make the flip real, not cosmetic.

**Boundary guidance (avoid the Infilling Trap)**

* Do **not** “cheat” by swapping masked high-impact words with synonyms, softened variants, or near-equivalents that keep the same class.
* When flipping **to Normal**: remove toxicity, soften stance, add civility/neutral phrasing.
* When flipping **to Offensive**: use generic insults/profanity without protected-group targeting.
* When flipping **away from Hate**: remove protected-group targeting and dehumanization; do not introduce new slurs or protected-group hate.

**Mask design (intelligent, diverse hypotheses)**

* Use varied masks across samples; do not always target the same word set.
* Preserve essential glue/function words unless minimal paraphrase is required, and never reintroduce excluded words.
* For boundary_infill, preferentially set mask=0 on likely high-attribution tokens (slurs, profanity, group mentions, threats, dehumanizing terms, strong intensifiers/negations) to give room for a true label shift.
""",
    },
